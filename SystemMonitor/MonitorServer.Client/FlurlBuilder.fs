...
let postJson<'a> (input: 'a) (request: IFlurlRequest) =
    request
        .PostJsonAsync(input)
        .ReceiveJson<'a>()
    |> Async.AwaitTask
...
let private create (env:Env) =
    let route = env.host + "/api/housing"
    route.WithHeader("x-api-key", env.apiKey)

let private fromEx<'a> (e: exn) (obj:'a) =
    if e = null then succeed obj
    else
        if e.InnerException <> null && e.InnerException :? FlurlHttpException then
...
let post<'a> (env:Env) (object:'a) =
    async {
        try
            let! result = create env
                          |> postJson object
